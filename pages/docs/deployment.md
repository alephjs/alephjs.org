...
```bash
$ deno run -A https://deno.land/x/aleph@v0.2.25/cli.ts start ${APP_DIR} --port 80
```
...
- **Build Command**: `curl -fsSL https://deno.land/x/install/install.sh | sh && /vercel/.deno/bin/deno run -A https://deno.land/x/aleph@v0.2.25/cli.ts build`
